...
public class ClientConfig {
    ...
    /**
     * clientId 为客户端IP+instance+(unitname 可选），
     * 如果在同一台物理服务器部署两个应用程序，应用程序岂不是clientId 相同，会造成混乱？
     *
     * 为了避免这个问题，
     * 如果instance 为默认值DEFAULT 的话，
     * RocketMQ 会自动将instance 设置为进程ID ，
     * 这样避免了不同进程的相互影响，
     * 但同一个NM 中的不同消费者和不同生产者在启动时获取到的MQC!ientlnstane 实例都是同一个。
     * 根据后面的介绍，MQClientInstance 封装了RocketMQ 网络处理API ，
     * 是消息生产者（ Producer ）、消息消费者( Consumer ）
     * 与NameServer、Broker 打交道的网络通道。
     * @return
     */
